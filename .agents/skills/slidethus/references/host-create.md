...
The first invocation freezes a schema-backed Host Create Session in a new workspace. Session 0.2 also retains the exact hash-bound Seed after direction review while Slide Specs is still pending, so a process restart cannot prepare or overwrite it again. A populated legacy workspace without this Session, or a 0.1 Session, is not silently adopted; use a new workspace until an explicit migration route exists. After answering any pending Host request, resume with only:
...
Every invocation returns `session_path`, `attempt_id` and `operation_path`. The operation closes as `host_input_required`, `rework_required`, `blocked`, `failed`, `design_ready`, render terminal, `calibration_office_evidence_pending`, `calibration_rework`, `calibration_approved`, `full_office_evidence_pending` or `candidate_office_review_pending`. Planning/calibration rework returns the earliest owner, exact immutable review/decision refs, open issue IDs and allowed next actions. These operational facts do not satisfy deck Gates.
...
| slide_specs | `content.slides`: current slide_id, content_blocks, one closed `representation`, visual_intent, density_budget, speaker_notes, editability_intent. `representation.kind` is text/typographic/image/chart/table/diagram/mixed and owns carrier reason, weight/density plus kind-specific semantic data (chart series/categories, table hierarchy, diagram nodes/edges, image role/asset strategy). It owns no coordinates or P6 styles. Code assigns stable Block/Representation IDs. |
| layout_plans | `content.plans`: current slide_id, layout_family, rationale, regions, `representation_ref`, ordered `focal_order` and a kind-matched `view`. View owns primary Region, negative space and only geometry/presentation choices such as chart orientation/labels, table header/emphasis, diagram ports/routing/label anchors or image fit/crop/focal point. It must not repeat representation semantics. Canvas remains 1280×720. |
| art_direction | No `content` wrapper: design_read, dials, direction, warnings, assumptions. `direction` follows `art_direction_packet.schema.json`; declare a supported page family/component variant and stable style IDs for every page/region, plus producer capability/prohibited combinations. Each page carries the frozen surface treatment. Unsupported or unconsumed style/view/routing decisions fail rather than falling back. |
| direction_review / semantic_planning_review / calibration_review / whole_deck_review | `proposal.findings`: immutable findings only, with slide_id, dimension, normalized_issue, severity, earliest_owner, location, finding, impact and recommended_fix. Do not submit `approved` or alter policy. The request supplies exact images, coverage and dependency context. |
| calibration_office_evidence | `proposal`: exactly application, build, profile, export_parameters and ordered pages (`slide_id`, workspace-local raster path). Application must be Microsoft PowerPoint. Artifact Tool preview paths are rejected. |
...
Read `providers/art-direction/taste/SKILL.md` completely before using it. Use static presentation-relevant principles, not automatic web UI constraints. Native prototypes remain isolated and never satisfy PPTX/release gates, but their file path and SHA-256 are machine-verified as the designed Create Seed's provenance. A fixed token proposal or a post-hoc PPTX is not a native prototype. The prototype must cover active body roles/material carriers, not only the cover, and receives a separate direction review; `Taste-generated` is provenance, not approval. Translate the approved composition grammar into representation, view geometry and P6 page designs; revise the earliest owner when the geometry or carrier changes.
...
For reviewed/critical work, invoke `slidethus create <workspace> --render`; the workflow chooses representative pages deterministically from the active roles/representations. Manual `--slide-id` selection is rejected because it could hide difficult pages. A controlled diagnostic may still use the declared legacy/lightweight route, but it cannot authorize reviewed/critical full render.

The sample and full attempts consume the same complete current Renderer IR and `scripts/render_artifact.mjs`. The receipt binds exact IR, compiler, backend/adapter/capability, assets/fonts and selection. Any identity drift invalidates the authorization. Fontconfig checks the requested font families and glyphs (`--font-match` may select the host binary); a declared substitution is recorded, never silently represented as the original font.

Current adapter: native primitive text/list, table, bar/line/pie/doughnut/area charts, editable node/edge diagrams, embedded PNG/JPEG images, and admitted non-semantic shape decoration. Chart output consumes the P5A categories/series plus P5B orientation/label/legend decisions; table output consumes explicit header rows/emphasized dimension; diagrams consume semantic node/edge IDs plus admitted routing/label anchors. There is no type inference, numeric coercion, equal-card semantic fallback or unsupported option dropping. Raster image/icon/diagram needs one manifested PNG/JPEG and explicit fit. SVG/vector assets, complex charts and rich text are not claimed. Unsupported content/clip/paginate paths fail explicitly.

Outputs live in a unique `outputs/host-candidates/candidate-*` directory: PPTX, per-page Artifact Tool previews/layouts, input snapshot and receipt. A started attempt closes as failed, timed out or `candidate_office_review_pending`; receipts are immutable. Registering Office evidence creates a new content-addressed `receipt-office-*.json` rather than rewriting the terminal receipt.

After sample render, export the exact selected pages through real Microsoft PowerPoint and answer the `calibration_office_evidence` request. The fixed independent reviewer submits findings; workflow derives the decision. On approval, plain resume performs the full attempt only with the identical IR/producer authorization. Export every full candidate page through PowerPoint, register it, and resume for whole-deck review. Critical/Major findings route to their earliest owner and cannot disappear on the same page hash by omission, downgrade or reviewer switch. A successful candidate still says `release_approved: false` until the formal delivery path is complete. Artifact Tool PNGs are never PowerPoint evidence, and no accepted output is overwritten.
