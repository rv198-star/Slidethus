...
- Consistency means a coherent visual language, not identical page backgrounds/compositions. Designed Create freezes a pre-layout Seed after Outline: it plans each page's visual carrier and surface treatment, and limits continuous plain surfaces. A plain page can be a deliberate contrast; a whole deck silently reduced to one flat color cannot satisfy the Seed. Samples must represent the actual content roles and difficult pages, not just attractive exceptions.
- Taste is the pinned default replaceable art-direction resource. Its original files stay untouched. Designed Create requires a workspace-local native prototype driven by Taste before P5A; only that is “Taste-generated”. The label proves this production provenance only; it is not an aesthetic-quality score or approval. Palette coherence, layout judgment and real-PowerPoint whole-deck review remain independent checks. Direct PPT authoring with its principles is “Taste-informed” and remains available only to explicit deterministic/legacy baselines. Prototypes never satisfy PPTX release Gates.

## Runtime and delivery limits

- Designed Create uses [host-create](host-create.md): one persisted intent Session → host reasoning → bound proposals → deterministic admission → one full-deck IR → one Artifact Tool adapter for samples and full candidates. After the initial invocation, plain `slidethus create <workspace>` resumes the Session; different request/Source values require an explicit Brief/Source revision and are never interpreted as informal continuation text.
...
- Host Create started/terminal operations are supporting runtime facts, not deck Gates. Planning rework must name the root phase, exact Planning Review and open issue IDs. A host candidate receipt is not the legacy Render Manifest and cannot satisfy G7/M5/release integration. Its PNGs are library previews, not Office renders. Declare the actual route and its measured/unknown editability.
