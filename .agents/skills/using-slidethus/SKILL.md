...
4. Use the [host Create entry](../slidethus/references/host-create.md) for designed production. The first invocation persists canonical intent, deterministic VisualAdmissionPolicy and fixed reviewer identity/capabilities; after each pending request, submit the context-bound response and resume with plain `slidethus create <workspace>`. Do not repeat or reinterpret the original request. Changed Brief, Source, provider or reviewer policy requires its explicit revision operation.
5. For reviewed/critical work, require full-deck Specs/Layout/P6 page designs and one complete admitted IR before rendering. Let the workflow choose representative slides, render them through the same Artifact Tool producer, register real PowerPoint page exports, and obtain the derived calibration authorization before the identical-IR full render. Do not hand-pick an attractive sample or use a second generator.
6. A phase completion is a continuation point, not the end of the user's full-deck request. Continue without asking “shall I do the next step?” when scope and approval mode permit. After full render, register all real PowerPoint pages and complete whole-deck review; Artifact Tool previews do not satisfy this step.
7. At review time, synthesize findings before repairs. Re-enter the earliest authorized phase and regenerate dependents; an unchanged page hash retains prior Critical/Major findings unless an immutable false-positive adjudication closes them. No per-phase autonomous repair loop and no unrelated framework edits.
...
- PowerPoint opening/visual review cannot be replaced by a library preview. Sample Office evidence can authorize the identical full attempt but does not satisfy G7/M5/release approval; full-deck Office evidence and P8 decision remain mandatory.
