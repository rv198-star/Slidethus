...
3. Plan → Design → Render → Review and handoff. Layout planning is mandatory; design cannot jump directly from prose to PPTX.
4. Use the [host Create entry](../slidethus/references/host-create.md) for designed production. On each pending request, the responsible phase supplies a context-bound proposal, resumes the same entry, and verifies the admitted artifact. Never substitute the deterministic planning baseline or a separate sample generator.
